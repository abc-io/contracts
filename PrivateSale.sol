...
contract PrivateSale is Utils, Owned {
    ...
    uint256 public constant DURATION = 900;                 // private sale duration
    uint256 public constant TOKEN_PRICE_N = 1;                  // initial price in wei (numerator)
    uint256 public constant TOKEN_PRICE_D = 100;                // initial price in wei (denominator)
    uint256 public constant MAX_GAS_PRICE = 50000000000 wei;    // maximum gas price for contribution transactions
    uint256 public constant MAX_BUY_AMOUNT = 100 ether;           // maximum buy amount
    uint256 public constant MIN_BUY_AMOUNT = 1 ether;           // minimum buy amount
    uint256 public constant FROZEN_TIME = 300;             // frozen time
    uint256 public constant TOTAL_ETHER_CAP = 300 ether;    // total ether cap
    ...
    */
    function PrivateSale(uint16 _period, SmartToken _token, uint256 _startTime, address _beneficiary)
        public
    ...
    */
    function review(address[] _approvedAddr) public
        ...
        returns(bool)
    {
        for(var i = 0; i < _approvedAddr.length; i++)
        {
            approved[_approvedAddr[i]] = true;
            Reviewed(_approvedAddr[i]);
        }
        return true;
    }
